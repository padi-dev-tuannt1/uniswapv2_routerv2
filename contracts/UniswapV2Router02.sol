...
contract UniswapV2Router02 is IUniswapV2Router01, Ownable {
        ...
        returns (uint[] memory amounts)
    {
        ...
        uint256 feeSwap = _calculateFeeSwap(msg.value);
    ...
    {
        require(path[path.length - 1] == WETH, "UniswapV2Router: INVALID_PATH");
        uint256 feeSwap = _calculateFeeSwap(amountIn);
        bool success = IERC20(path[0]).transferFrom(
            msg.sender,
            feeRecipient,
            feeSwap
    ...
    function quote(
        ...
        uint reserveA,
        uint reserveB
    ) public view virtual override returns (uint amountB) {
        return
            UniswapV2Library.quote(
                amountA - _calculateFeeSwap(amountA),
                reserveA,
                reserveB
            );
    }
    ...
    function getAmountOut(
        ...
        return
            UniswapV2Library.getAmountOut(
                amountIn - _calculateFeeSwap(amountIn),
                reserveIn,
                reserveOut
            );
    }
